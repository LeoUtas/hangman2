...
def draw_button(*args):
    ...
    for _ in letters:
        ...
        if visibility:
            ...
            pygame.draw.rect(
                window, Designs.button_color, space_line, border_radius=10, width=2
            )

    text = Designs.copyright_text_font.render(
        "names source: en.wikipedia.org, GUI idea: TechwithTim", 1, Designs.text_color
    )
...
def greeting():
    window.fill(Designs.bg_color)
    text = Designs.promt_text_font.render(
        "This hangman game is about guessing", 1, Designs.text_color
    )
    window.blit(
        text, (WIDTH / 2 - text.get_width() / 2, HEIGHT / 2.8 - text.get_height() / 2)
    )
    text = Designs.promt_text_font.render(
        "names in Harry Potter", 1, Designs.text_color
    )
    window.blit(
...
def display_messages(*args):
    ...
    if args[2] == False:
        ...
        window.blit(
            text,
            (WIDTH / 2 - text.get_width() / 2, HEIGHT / 2.6 - text.get_height() / 2),
        )
    else:
        text = Designs.promt_text_font.render(args[0], 1, args[1])
        window.blit(
        ...
        window.blit(
            text,
            (WIDTH / 2 - text.get_width() / 2, HEIGHT / 1.8 - text.get_height() / 2),
        )
...
def promt():
    window.fill(Designs.bg_color)
    promt = Designs.promt_text_font.render(
        "'Yes'to play or 'No' to quit?", 1, Designs.text_color
    )
    window.blit(
        promt,
        (WIDTH / 2 - promt.get_width() / 2, HEIGHT / 2.6 - promt.get_height() / 2),
    )
    yes = Designs.promt_text_font.render("Yes", 1, Designs.text_color)
    window.blit(
        yes, (WIDTH / 3 - yes.get_width() / 2, HEIGHT / 1.8 - yes.get_height() / 2)
    )
    no = Designs.promt_text_font.render("No", 1, Designs.text_color)
    window.blit(
        no, (WIDTH * 2 / 3 - no.get_width() / 2, HEIGHT / 1.8 - no.get_height() / 2)
    )

    yes_pos = (
        WIDTH / 3 - yes.get_width() / 2 + 30,
        HEIGHT / 1.65 - yes.get_height() / 2,
    )
    no_pos = (
        WIDTH * 2 / 3 - no.get_width() / 2 + 22,
        HEIGHT / 1.65 - no.get_height() / 2,
    )

    pygame.draw.circle(
        window, Designs.text_color, (yes_pos[0], yes_pos[1]), RADIUS + 22, 3
    )
    pygame.draw.circle(
        window, Designs.text_color, (no_pos[0], no_pos[1]), RADIUS + 22, 3
    )
    pygame.display.update()
...
def game_play():
    ...
    run = True
    path_to_score_CSV = os.path.join("data", "score_data.csv")
    ...
    while run:
        ...
        if hangman_stage == 6:
            run = False
            display_messages(
                "pOops, you killed the man!", Designs.text_color, f"the word is {WORD}"
            )
            print(WORD)
            return score
